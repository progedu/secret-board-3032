  ...
  switch (req.method) {
    ...
    case 'POST':
      ...
      req.on('data', (chunk) => {
        ...
        console.info('デコード'+decoded);
        const dataArray = decoded.split('&');
        console.info('データアレイの長さ'+dataArray.length);
        if (dataArray.length > 2)
        {
          for ( var i = 1; i < dataArray.length-1; i++){
            dataArray[0] += '&'+dataArray[i];
          }
          dataArray[1] = dataArray[dataArray.length-1]
        }
        console.info('content'+dataArray[0]);
        const content = dataArray[0] ? dataArray[0].split('content=')[1] : '';// content= 投稿内容&oneTimeToken=トークン本体
